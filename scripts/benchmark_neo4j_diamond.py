...
import time
import neo4j_queries.diamond_queries as diamond_queries
...
if __name__ == "__main__":
    database_name = 'diamond1000'
    with open('NEO4J_DIAMOND_RESULTS.tsv', 'w') as results_file:
        results_file.write('query\tresults\tstatus\ttime\n')
        driver = GraphDatabase.driver('bolt://127.0.0.1:7687')
        with driver.session(database=database_name) as session:
            for query in diamond_queries.queries:
                print(query)
