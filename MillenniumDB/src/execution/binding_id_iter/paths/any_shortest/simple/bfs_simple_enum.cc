...
void BFSSimpleEnum::begin(BindingId& _parent_binding) {
    ...
    open.emplace(automaton.start_state,
                 start_object_id,
                 nullptr,
                 true,
                 ObjectId::get_null());

    visited.emplace(automaton.start_state,
                    start_object_id,
                    nullptr,
                    true,
                    ObjectId::get_null());
...
bool BFSSimpleEnum::next() {
    ...
    while (open.size() > 0) {
        ...
        if (is_first) {
            ...
            if (current_state.automaton_state == automaton.start_state &&
                automaton.is_final_state[automaton.start_state])
            {
                results_found++;
        ...
        else if (automaton.is_final_state[current_state.automaton_state]) {
            results_found++;
            auto path_object_id = path_manager.set_path(visited.find(current_state).operator->(),
                                                        path_var);
...
unique_ptr<BptIter<4>> BFSSimpleEnum::set_iter(const RPQ_DFA::Transition& transition,
