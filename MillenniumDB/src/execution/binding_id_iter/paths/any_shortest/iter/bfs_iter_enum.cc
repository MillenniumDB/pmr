...
bool BFSIterEnum::next() {
    ...
    if (first_next) {
        ...
        if (automaton.is_final_state[automaton.start_state]) {
            auto reached_key = SearchState(automaton.start_state,
    ...
    while (open.size() > 0) {
        ...
        if (state_reached != visited.end()) {
            ...
            if (automaton.is_final_state[state_reached->automaton_state]) {
                auto found_reached_final = reached_final.find(state_reached->node_id.id);
    ...
    BFSIterEnum::current_state_has_next(const SearchState* current_state)
{
    ...
    while (current_transition < automaton.from_to_connections[current_state->automaton_state].size()) {
        ...
        while (child_record != nullptr) {
            SearchState next_state_key(transition.to,
            ...
            if (inserted_state.second) { // Inserted_state.second = true if state was inserted in visited
