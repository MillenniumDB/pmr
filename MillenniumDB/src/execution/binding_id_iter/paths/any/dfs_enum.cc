...
bool DFSEnum::next() {
    ...
    if (first_next) {
        ...
        if (automaton.is_final_state[automaton.start_state]) {
            auto current_key =
              Paths::AnyShortest::SearchState(automaton.start_state, open.top().object_id, nullptr, true, ObjectId::get_null());
    ...
    while (open.size() > 0) {
        ...
        if (reached_state != visited.end()) {
            ...
            if (reached_state->automaton_state == automaton.start_state) {
