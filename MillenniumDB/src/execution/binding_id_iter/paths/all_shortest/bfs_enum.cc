...
bool BFSEnum::expand_neighbors(const SearchState* current_state) {
            ...
            if (visited_search != visited.end()) {
                ...
                if (visited_search->distance == next_state.distance) {
                    ...
                    if (automaton.is_final_state[next_state.automaton_state]) {
                        auto found_reached_final = reached_final.find(next_state.node_id.id); // must exist
                        if (found_reached_final->second == next_state.distance)
                        {
