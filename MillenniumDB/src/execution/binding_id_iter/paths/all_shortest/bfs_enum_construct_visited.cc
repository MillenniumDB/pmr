#include "bfs_enum_construct_visited.h"
...
BFSEnumConstructVisited::BFSEnumConstructVisited(ThreadInfo* thread_info,
                                                 VarId       path_var,
                                                 Id          start,
                                                 VarId       end,
                                                 RPQ_DFA     automaton) :
...
void BFSEnumConstructVisited::begin(BindingId& _parent_binding) {
...
bool BFSEnumConstructVisited::next() {
...
robin_hood::unordered_node_set<SearchState>::iterator
  BFSEnumConstructVisited::current_state_has_next(const SearchState* current_state)
{
...
void BFSEnumConstructVisited::set_iter(const SearchState* current_state) {
...
void BFSEnumConstructVisited::reset() {
...
void BFSEnumConstructVisited::assign_nulls() {
    parent_binding->add(end, ObjectId::get_null());
}


void BFSEnumConstructVisited::analyze(std::ostream& os, int indent) const {
    os << std::string(indent, ' ');
    os << "Paths::AllShortest::BFSEnumConstructVisited(bpt_searches: " << bpt_searches << ", found: " << results_found << ")";
}
