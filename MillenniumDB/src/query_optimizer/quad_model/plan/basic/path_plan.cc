...
#include "execution/binding_id_iter/paths/all_shortest/bfs_check_construct_visited.h"
#include "execution/binding_id_iter/paths/all_shortest/bfs_check_count.h"
#include "execution/binding_id_iter/paths/all_shortest/bfs_enum.h"
#include "execution/binding_id_iter/paths/all_shortest/bfs_enum_construct_visited.h"
#include "execution/binding_id_iter/paths/all_shortest/bfs_enum_count.h"
...
unique_ptr<BindingIdIter> PathPlan::get_binding_id_iter(ThreadInfo* thread_info) const {
    ...
    case PathSemantic::ALL : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                ...
                throw QuerySemanticException("property paths with ALL semantics must have at least 1 node fixed.");
            }
        }
    }
    case  PathSemantic::ALL_COUNT : {
        if (from_assigned) {
            ...
            if (to_assigned) {
                // bool case
                return make_unique<Paths::AllShortest::BFSCheckCount>(thread_info,
                                                                      path_var,
                                                                      from,
                                                                      to,
                                                                      automaton);
                ...
                // enum starting on from
                return make_unique<Paths::AllShortest::BFSEnumCount>(thread_info,
                                                                     path_var,
                                                                     from,
                                                                     std::get<VarId>(to),
                                                                     automaton);
    ...
    case  PathSemantic::ALL_FILL_VISITED : {
        if (from_assigned) {
            auto automaton = path.get_rpq_automaton(str_to_object_id_f);
            if (to_assigned) {
                // bool case
                return make_unique<Paths::AllShortest::BFSCheckConstructVisited>(
                    thread_info,
                    path_var,
                    from,
                    to,
                ...
                // enum starting on from
                return make_unique<Paths::AllShortest::BFSEnumConstructVisited>(
                    thread_info,
                    path_var,
                    from,
                    std::get<VarId>(to),
            ...
            if (to_assigned) {
                ...
                // TODO: allow no-nullable unfixed paths
                throw QuerySemanticException("property paths with ALL_FILL_VISITED semantics must have at least 1 node fixed.");
            }
        }
    }
    }
